...
- **알림 설정**: 리마인더를 몇 분 전에 받을지(0~60분), 공유·댓글·주간 리포트·응원 알림을 각각 켜고 끌 수 있다
...
- **첫 사용 안내**: 가입 직후 3단계(오늘 계획 → 목표일 → 타이머 체험)로 3분 안에 첫 기록까지 안내.
  '오늘 할 일' 부터 묻는다 - 첫 화면이 "목표가 뭔가요?" 이면, 아직 이 도구가 뭘 해 주는지 모르는 사람이 거기서 멈춘다.
...
- **플래너**: 일간/주간/월간 뷰 (로그인 회원 본인 것만), 완료 토글,
  분류(코딩테스트·자소서·면접 등), 반복 일정(매일/평일/매주), **예상 소요 시간**(칩 한 번이면 채워진다).
  주간 뷰는 일곱 칸에 바로 적을 수 있고, '지난주 계획 가져오기' 로 한 주를 통째로 옮길 수 있다
...
- **학습 타이머**: 계획을 실행할 때 시작/종료를 눌러 **실제 공부한 시간**을 기록.
  포모도로(25분 집중 / 5분 휴식, 네 번마다 15분)를 켜고 끌 수 있다 - 인강 한 편이 70분이면 25분에 끊기는 것이 방해다.
...
- **지원 현황**: 어느 회사에, 지금 어느 단계로(서류·코테·1차·최종·결과 대기), 언제까지인지를 한자리에.
  마감은 D-Day 와 **같은 줄**에 뜨고, 단계는 학습 분류와 이어진다 -
  코테 단계인 회사가 셋인데 이번 주 코딩테스트 학습이 0시간이면, 분류별 통계가 비로소 판단이 된다.
  스스로 그만둔 곳은 '포기' 로 남긴다(불합격으로 적게 하면 실제보다 많이 떨어진 기록이 남는다)
- **게시판**: CRUD + 검색(제목/제목+내용/작성자 닉네임) + 페이징, 읽기는 공개·쓰기는 로그인 필요.
  분류(자유/공고/후기/질문) 탭, 좋아요, 조회수, 마크다운 본문(렌더 후 허용 목록으로 살균해 XSS 차단)
- **그룹 응원**: 순위표에서 👏 한 번. 하루에 한 사람당 한 번이다 - 순위는 상위권을 더 뛰게 하지만
  하위권을 조용히 떠나게 하므로, 반대 방향으로 작동하는 버튼이 하나는 있어야 한다
- **주간 리포트 알림**: 일요일 저녁, "이번 주 3시간, 완료율 50% — 회고 쓰러 가기".
  기록이 없는 주에는 보내지 않는다 ("이번 주 0시간" 은 요약이 아니라 질책이다)
...
- **캘린더 연동**: 구독 주소(iCal)를 구글 캘린더에 등록하면 계획이 자동으로 따라온다. CSV 로 내려받아 엑셀에서 열 수도 있다.
  반대로 다른 캘린더에서 내보낸 `.ics` 를 **가져올** 수도 있다 - 줄 접기·UTC/TZID·이스케이프를 풀어 읽고,
  같은 날 같은 제목은 건너뛴다. 반복 일정은 가져오지 않고 몇 개를 못 가져왔는지 알려 준다
...
이 프로젝트의 부분 갱신은 이렇게 동작한다: **서버에서 HTML 조각을 받아 DOM 에 끼운다.**

```
POST /api/plans          → { plan: {...}, progress: {...} }
GET  /plans/{id}/row     → <li class="plan-card"> ... </li>   (plans/row :: planRow)
```

그건 정확히 HTMX 가 하는 일이라, 한 화면(일간 뷰의 한 줄 추가)을 옮겨 보고 견주었다.

| | 지금 (fetch + 프래그먼트) | HTMX |
|---|---|---|
| 마크업 | `<form id="quick-add">` | `hx-post="/plans" hx-target="#plan-list" hx-swap="beforeend"` |
| JS | `plans.js` 의 한 줄 추가 블록 **48줄** | 0줄 |
| CSRF | `UI.csrfHeaders()` | `hx-headers` 또는 메타 태그 한 줄 |
| 서버 | JSON + 조각 두 경로 | 조각 하나면 된다 |
| 삽입 위치 | `insertSorted()` 가 서버 정렬 규칙을 흉내 낸다 | **여전히 필요하다** (`hx-swap` 은 끝에 붙일 뿐이다) |
| 진행 카운터 | 같은 응답의 `progress` 로 갱신 | 조각을 하나 더 보내거나(OOB swap) 서버가 두 군데를 갱신해야 한다 |
| 실패 안내 | `UI.toast(await UI.readError(res))` | `htmx:responseError` 를 받아 같은 토스트를 부른다 |

**48줄이 사라지는 대신, 정렬과 카운터가 서버 쪽 복잡함으로 옮겨 간다.** 그리고 토스트·확인창·
중복 제출 방지(`ui.js`, 250줄)는 어느 쪽이든 그대로 남는다 - 그것들은 통신 방식과 상관이 없다.

그래서 **지금은 쓰지 않는다.** 두 가지 이유다:

1. **얻는 것이 생각보다 작다.** 라이브러리 하나(14KB)와 새로운 속성 어휘를 들이는 대가로
   줄어드는 것이 한 화면에서 48줄인데, 그중 절반은 사라지지 않고 서버로 옮겨 간다.
2. **직접 구현한 것이 설명거리다.** 서버가 조각을 그리고 JS 가 끼우는 구조를 **왜** 그렇게 했는지
   (한 줄의 생김새가 두 곳에 있어 어긋났던 일) 코드로 보여 줄 수 있다.

다시 저울에 올릴 시점은 분명하다: **부분 갱신 화면이 네다섯 개를 넘어가고**,
`insertSorted` 같은 조각이 화면마다 반복되기 시작할 때다. 지금은 두 곳(일간·주간)이다.
...
CI 에는 **Lighthouse** 도 붙어 있다(`.github/workflows/lighthouse.yml`).
화면 규칙은 문서와 테스트로 꽤 지켜 왔지만 그 규칙들은 "무엇을 그리는가" 에 관한 것이라,
대비가 모자란 색 하나·라벨 없는 버튼 하나는 어느 테스트에도 걸리지 않고 들어온다.
접근성 0.95 는 **error**(규칙이 분명하고 흔들리지 않는다), 성능 0.85 는 **warn** 이다 -
공용 러너는 실행마다 편차가 커서, error 로 두면 사람들은 경고를 끄는 대신 검사 자체를 지운다.
...
├── common/            # GlobalExceptionHandler (404/403/500), web/PageBlock, time/{ServiceZone,ReadableDuration}
...
├── calendar/          # 캘린더 내보내기·가져오기 (iCal 구독·CSV·.ics 읽기, 외부 라이브러리 없음)
...
- 쿠키 속성은 `CookiePolicy` 한 곳에서 정합니다 - HttpOnly · SameSite=Lax · Secure.
  `Secure` 는 설정값(`app.cookie.secure`, 환경변수 `APP_COOKIE_SECURE`)이며 **운영(https)에서는 반드시 켜세요**.
  로컬(http)에서 켜면 브라우저가 쿠키를 아예 보내지 않아 로그인이 조용히 실패합니다.
  `SameSite=Strict` 는 쓰지 않습니다 - 제공자에서 돌아오는 소셜 로그인 콜백이 깨집니다.
- 로그인 성공 시 두 개의 쿠키를 발급합니다.
...
- 소셜 로그인의 인가 요청(state 등)은 세션 대신 쿠키에 담습니다(무상태를 유지하기 위해).
  담을 때는 필요한 필드만 **JSON** 으로 옮기고 **HMAC 으로 서명**합니다(`AuthorizationRequestCodec`).
  자바 직렬화를 쓰지 않는 이유는 쿠키가 곧 공격자가 정할 수 있는 입력이기 때문이고,
  서명하는 이유는 서명이 없으면 공격자가 자기 state 를 심어 피해자를 남의 계정으로 로그인시킬 수 있기 때문입니다.
- 소셜 계정에는 비밀번호가 없습니다(`!social`). 비밀번호 변경·재설정 경로에서 제외되며,
  탈퇴 확인은 비밀번호 대신 닉네임 입력으로 받습니다.
- 시크릿은 `jwt.secret`(환경변수 `JWT_SECRET`)로 주입하며 운영 환경에서는 반드시 교체하세요.
  인가 요청 서명 키는 이 값에서 용도 라벨을 섞어 파생시킵니다(같은 키를 두 목적으로 쓰지 않습니다).
