...
| 시간대 | `Clock.systemDefaultZone()` 을 쓰지 않는다. JVM 이 뜬 기계의 시간대를 따르므로 로컬(KST)과 배포(대개 UTC)에서 **"오늘" 이 다른 날**이 된다 - 날짜가 축인 이 앱에서는 자정에 적은 일정이 어제 것으로 들어간다. `ServiceZone` 한 곳이 정하고, `@Scheduled(cron=...)` 에도 `zone = ServiceZone.ID` 를 적는다(빠뜨리면 새벽 4시 정리가 오후 1시에 돈다). `TZ` 환경변수에 기대지 않는다 - 빠뜨린 배포가 다시 기계를 따른다 |
| 현재 시각 | `LocalDate.now()`/`LocalDateTime.now()` 를 직접 부르지 않는다. 컨트롤러도 `Clock` 을 받는다 - 시간대가 따라오고, 한 요청 안에서 '오늘' 이 두 값이 되지 않는다(`daily()` 가 now() 를 세 번 불렀다) |
| 비밀값의 기본값 | 설정에 운영에서도 통하는 기본값을 두지 않는다. `jwt.secret` 은 기본값이 없어 `JWT_SECRET` 없이는 부팅이 실패한다 - 편의는 `local` 프로필이 맡고, `spring.profiles.default: local` 이 받아서 바로 실행되게 한다. H2 콘솔과 같은 원칙이다: 잊으면 안 되는 구조 대신, 잊으면 뜨지 않는 구조 |
| 미인증 응답 | 로그인이 필요한 요청을 전부 `/login` 으로 302 시키면 안 된다. 브라우저는 리다이렉트를 **따라가** 200 + HTML 을 받으므로 `res.ok` 가 참이 되어 JS 의 오류 처리가 통째로 무력해지고, `EventSource` 는 영원히 재접속한다. `Accept` 로 갈라 데이터를 원하는 요청에는 401 을 준다(`UnauthenticatedEntryPoint`) |
| 인덱스 | "누구의 것을 어느 날짜로 찾는가" 는 그대로 인덱스가 된다. 엔티티가 아니라 마이그레이션에 적는다(테스트는 엔티티로 스키마를 만들어 인덱스를 보지 않으므로, 적는 자리를 헷갈리면 운영에만 있는 규칙이 된다) |

## 화면 규칙 (UI 일관성)

세 번의 서버 검토 뒤 화면을 훑었더니, 같은 종류의 어긋남이 반복되고 있었다 —
**같은 것을 그리는 코드가 여러 벌이고, 그중 하나만 자란다.** 아래는 그때 정한 규칙이다.

| 항목 | 규칙 |
|---|---|
| **한 줄의 출처** | 같은 데이터를 여러 화면에서 그린다면 프래그먼트 하나에서 그린다. 일정 한 줄이 다섯 벌이던 때 검색 화면의 체크 상자는 `<span>` 이라 **모양만 있고 눌러도 완료가 되지 않았다**. 화면마다 필요한 차이는 프래그먼트 인자로 받는다(`planRow(showDate)`) |
| **생김새가 같으면 동작도 같다** | 주간 뷰의 일정은 `<div>`, 월간 뷰의 같은 자리는 링크였다. "보이는데 안 되는 것" 은 사용자가 자기 조작이 실패했다고 느끼므로 없는 것보다 나쁘다 |
| **날짜 포맷** | 용도별로 정해진 것만 쓴다(`DateFormatConsistencyTest` 가 지킨다). 한때 열두 가지였고 `M/d(E)` 와 `M/d (E)` 처럼 **괄호 앞 공백만 다른 것**까지 있었다 |
| **primary 는 화면에 하나** | 그 화면에서 사용자가 가장 자주 하려는 일에만 붙인다. 대등한 행동이 나란히 있으면(그룹 만들기 / 초대 코드로 가입) 둘 다 붙인다 — 한쪽만 강조하면 다른 쪽이 부차적으로 보인다 |
| **버튼 클래스 순서** | `btn` → 크기(`btn-sm`/`btn-lg`) → 색(`btn-primary`/`btn-danger`) → 기능(`btn-timer` 등) |
| **같은 동작에 같은 말** | '모두 읽음'/'모두 읽음으로', '모두 삭제'/'전체 삭제', '삭제'/'지우기' 가 섞여 있었다. 확인 문구는 **"~할까요?"** 로 통일한다(탈퇴만 "~하시겠습니까?" 였다) |
| **탭 제목** | `화면 이름 · 스터디플랜`. 앞에 화면 이름을 두는 이유는 탭이 좁아지면 뒤부터 잘리기 때문이다 |
| **모바일 내비** | 헤더 메뉴(7개)가 720px 이하에서 감춰지므로, 탭바(5칸)에서 **모든 메뉴에 닿을 수 있어야** 한다. 넘치는 것은 '더보기' 에 담는다 — 예전에는 그냥 빼서 D-Day·그룹·공유 플랜이 폰에서 사라졌다(`MobileNavigationTest` 가 지킨다) |
| **넓은 것은 감싼다** | `min-width` 를 가진 내용(게시판 표·달력·주간 격자)은 `.scroll-x` 로 감싼다. 안 감싸면 본문 전체가 가로로 밀리고 고정 헤더·탭바까지 함께 밀린다. 래퍼 이름은 **하나**다 — `.table-wrap` 과 `.scroll-x` 가 같은 정의로 공존하던 때가 있었다 |
| **빈 상태에 다음 행동** | 목록이 비었을 때 "없습니다" 로 끝내지 않는다. 거기서 할 수 있는 일을 한 줄 붙인다 |
| **중복 제출** | 서버로 가는 폼도 잠근다(`ui.js` 의 전역 submit 리스너). `withBusy` 는 fetch 경로에만 붙어 있었는데, 화면이 통째로 넘어가는 폼이 훨씬 많고 그쪽이 "안 눌렸나" 하고 다시 누르기 쉽다 |
| **`th:replace` 우선순위** | `th:if`/`th:each` 를 `th:replace` 와 **한 태그에 같이 쓰지 않는다**. replace 가 먼저 처리돼 조건과 반복이 무시된다 — `<th:block>` 으로 감싼다 |
