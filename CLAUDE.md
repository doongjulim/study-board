...
| @ModelAttribute 의 이름 | 이름을 생략하면 <b>타입 이름</b>에서 뽑힌다(`JobApplicationForm` → `jobApplicationForm`). 화면이 다른 이름으로 찾으면 검증 실패로 화면을 다시 그릴 때만 터진다 - 정상 경로에서는 보이지 않아 늦게 발견된다. 폼 이름은 적어 둔다 |
| 밖에서 들어오는 값 | 우리 폼을 거치지 않는 입력(캘린더 가져오기)은 `@Size` 검증이 걸리지 않는다. 컬럼 상한은 <b>들이는 쪽</b>에서 지킨다 - 넘치면 커밋 때 터지면서 같은 트랜잭션의 다른 저장까지 함께 롤백된다(알림 메시지에서 이미 겪었다). 길이 상수는 `Plan.MAX_TITLE_LENGTH` 처럼 엔티티 한 곳에 두고 컬럼 선언과 폼 검증이 함께 쓴다 |
| 남의 시간대 | 외부 캘린더의 `DTSTART` 는 `Z`(UTC)이거나 `TZID` 를 달고 온다. 적힌 숫자를 그대로 읽으면 오전 10시 수업이 새벽 1시로 들어온다. 파라미터 이름만 대문자로 견주고 <b>값은 적힌 그대로</b> 쓴다 - `TZID=Asia/Seoul` 을 통째로 대문자로 바꾸면 그 시간대를 찾지 못한다 |
| 로그인 없이 받아 가는 것 | 파비콘·`manifest`·`og:image` 는 <b>브라우저와 메신저가 로그인하지 않은 채</b> 가져간다. `permitAll` 에서 빠지면 로그인 화면으로 302 되고, 받는 쪽은 그것을 이미지로 읽으려다 실패한다. 화면 안에서는 아무 표도 나지 않아 오래 남는 종류다 - `SiteHeadTest` 가 실제로 그려진 head 에서 주소를 뽑아 익명으로 받아 본다 |
| 통계가 조용히 사라지는 것 | 화면이 `값 > 0` 일 때만 지표를 그리면, 그 값을 채우는 **입력 경로가 없을 때 기능 자체가 없어진 줄도 모르게** 된다. 계획 시간은 시작·종료 시각이 둘 다 있어야 잡혔는데 주 입력 경로인 '한 줄 추가' 는 시각을 받지 않았다 - 대부분의 사용자에게 '계획 대비 실행률' 이 한 번도 보이지 않았다. **지표를 만들면 그 값을 채우는 길이 주 입력 경로에 있는지 함께 본다** |
...
| **시간 표기** | 분을 시·분으로 적는 규칙은 `ReadableDuration` 하나가 정한다(`DurationFormatConsistencyTest` 가 지킨다). 순위표는 "45분"·"1시간" 인데 통계·대시보드·주간 리포트는 시와 분을 각자 이어 붙여 **"0시간 45분"·"1시간 0분"** 이었다 — 사람이 쓰지 않는 말이고, 같은 숫자가 화면마다 다르게 보였다 |
...
| **입력칸에는 라벨** | `placeholder` 는 라벨이 아니다 - 값을 적는 순간 사라지고, 스크린리더에는 "편집, 비어 있음" 으로만 들린다. 화면에 두기 싫으면 `<label class="sr-only">` 로 감춘다. 아이콘만 있는 버튼은 `aria-label` 로 말한다 (`AccessibleFormsTest` 가 공개 화면을 훑고, CI 의 Lighthouse 가 한 번 더 본다) |
...
| **공유 미리보기·아이콘** | 레이아웃이 기본값을 넣고, 더 나은 재료가 있는 화면만 `OpenGraphMeta` 로 덮어쓴다. 프래그먼트 인자를 늘리면 이 레이아웃을 쓰는 스무 개 화면을 전부 고쳐야 한다. `og:image` 는 <b>절대 주소</b>여야 한다 - 미리보기를 만드는 쪽은 우리 서버가 아니다(`SiteMetaAdvice` 의 `baseUrl`). 아이콘 그림은 `tools/generate_icons.py` 한 곳에서 만든다 |
| **자주 안 여는 설정은 접는다** | 캘린더 연동·탈퇴처럼 한 번 보고 마는 것은 `<details>` 로 접는다. 되돌릴 수 없는 행동이 매일 쓰는 화면에 상주하지 않게 하는 뜻도 있다. 접힌 구역을 가리키는 앵커는 `ui.js` 가 열어 준다(JS 가 없으면 접힌 채로 도착하고 한 번 누르면 열린다) |
